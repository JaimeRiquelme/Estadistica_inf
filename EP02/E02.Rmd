...
datos <- read.csv2("/Users/maxito/Desktop/Nivel 6/Inferencial/Ejercicios/E02/EP02 Data.csv")
```
...
Así, como el valor p obtenido es mayor a nuestro nivel de significancia, por lo que la evidencia nos sugiere rechazar HA en favor de la hipótesis H0 y se puede afirmar con 95% de confianza que el tiempo promedio previo es de *19,4*.
...
Así, como el valor p obtenido es menor a nuestro nivel de significancia, por lo que la evidencia nos sugiere rechazar HA en favor de la hipótesis H0 y se puede afirmar con 95% de confianza que la diferencia entre las medias previas y posteriores es de 1,3 segundos.
...
Ahora, una vez calculada la prueba t de Student para muestras independientes se obtuvo un valor p = `r prueba_t$p.value`, que sugiere rechazar la H0 siendo este mucho menor a nuestro nivel de significancia. Concluyendo, podemos asegurar que con un 95% de confianza la media de las diferencias entre las razas es menor a 5.2.
...
exitos_raza_blanca <- count(datos_raza_blanca) * proporcion_blanca
fracasos_raza_blanca <- count(datos_raza_blanca) * (1 - proporcion_blanca)

# cálculo de la cantidad de éxitos y fracasos para la raza oriental
exitos_raza_oriental <- count(datos_raza_oriental) * proporcion_oriental
fracasos_raza_oriental <- count(datos_raza_oriental) * (1 - proporcion_oriental)
```
...
```{r, echo = FALSE}
cantidad_blanca <- count(datos_raza_blanca)
cantidad_oriental <- count(datos_raza_oriental)

n <- c(cantidad_blanca, cantidad_oriental)
exitos <- c(exitos_raza_blanca, exitos_raza_oriental)
alpha <- 0.05
