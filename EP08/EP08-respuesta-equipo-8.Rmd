...
F_fuera_de_Ho <- function(df, A, B, C){
...
f_obs <- F_fuera_de_Ho(muestra_2, n_Talagante, n_Maipo, n_Cordillera)

print(f_obs)
```
Sabemos que no podemos realizar boostraping a estos datos  ya que no representan la hipotesis nula, por lo cual es necesario determinar las 3 mismas provincias pero donde estas cuenten con promedios y varianzas iguales.
...
#Deplazamineto de los valores para igualar medidas (media y varianza)
...
```{r}
# Definir número de remuestreos
B <- 1000  # Puedes ajustar este número

# Realizar el remuestreo para cada provincia
set.seed(9028)  # Para reproducibilidad
remuestreo_Cordillera <- lapply(1:B, function(i) 
  sample(n_Cordillera, length(n_Cordillera), replace = TRUE))

remuestreo_Maipo <- lapply(1:B, function(i) 
  sample(n_Maipo, length(n_Maipo), replace = TRUE))

remuestreo_Talagante <- lapply(1:B, function(i) 
  sample(n_Talagante, length(n_Talagante), replace = TRUE))
...
F_crit <- quantile(distribucion, 0.95)
 cat("F crítico con 95% de confianza:", round(F_crit, 3), "\n")
  ...
  numerador2 <- sum(distribucion > f_obs)
 valor_p2 <- (numerador2 + 1) / (B + 1)
 cat("P-valor:", round(valor_p2, 3))
```
Verificamos utilizando la funcion t1way de la libreria WRS2

```{r}
library(WRS2)

t1waybt(ytot ~ provincia, data = muestra_2, R = 1000)
```


## Conclusión

Luego de realizar el remuestreo con bootstrapping y aplicar ANOVA, se obtuvo un valor p de 0.124, lo que nos indica que no existe evidencia suficiente para rechazar la hipótesis nula. Por lo tanto, no podemos afirmar que exista una diferencia significativa en las medias de los ingresos totales para hombres y mujeres en las provincias de Maipo, Cordillera y Talagante.
...
boot_dif_Cord_Maipo <- sapply(1:B,
                            function(i) dif_medias(muestra_2_igual_medidas,
                                                       remuestreo_Cordillera[[i]],
                                                       remuestreo_Maipo[[i]]))
 boot_dif_Cord_Talag <- sapply(1:B,
                            function(i) dif_medias(muestra_2_igual_medidas,
                                                       remuestreo_Cordillera[[i]],
                                                       remuestreo_Talagante[[i]]))
 boot_Maipo_Talagante <- sapply(1:B,
                            function(i) dif_medias(muestra_2_igual_medidas,
                                                       remuestreo_Maipo[[i]],
                                                       remuestreo_Talagante[[i]]))
...
intervalo_confianza_cord_maip <- quantile(boot_dif_Cord_Maipo, c(0.025, 0.975))
intervalo_confianza_cord_talag <- quantile(boot_dif_Cord_Talag, c(0.025, 0.975))
intervalo_confianza_maipo_talag <- quantile(boot_Maipo_Talagante, c(0.025, 0.975))
...
valor_p.A.C <- (sum(abs(boot_dif_Cord_Maipo) > abs(dif_Cord_Maipo)) + 1) / (B + 1)
 valor_p.A.M <- (sum(abs(boot_dif_Cord_Talag) > abs(boot_dif_Cord_Talag)) + 1) / (B + 1)
 valor_p.C.M <- (sum(abs(boot_Maipo_Talagante) > abs(boot_Maipo_Talagante)) + 1) / (B + 1)
...
Cordillera vs Maipo: [ -168167.4 146467.7 ] Cordillera vs Talagante: [ -202795.4 158307.5 ] Maipo vs Talagante: [ -210636.3 180896.6 ]
