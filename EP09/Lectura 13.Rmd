...
Diremos que un modelo es eneralizable si para un conjutno de datos nuevo consigue predicciones con una calidad similar a la que consigue con los datos usados en su construccion.

La estrategia mas frecuente es la validacion cruzada, donde se separa en dos fragmentos:

- Conjunto de entrenamiento: suele contener entre el 80 y 90% de las observaciones, escogidas de manera aleatorioa y se emplea para ajustar la recta con el metodo de minimos cuadrados.

- Conjunto de prueba_ contiene el 10 a 30% restante de la muestra, y se usa para evaluar el modelo con datos nuevos.

La idea del modelo es evaluar como se comporta el modelo con datos que no ha visto previamente, en comparacion al comportamiento con el conjunto de entrenamiento.
...
Pero puede ocurrir que por cosas randoms, el conjunto de prueba quede con observaciones que no represenan adecuadamente la muestra de datos original, llevando a una estimacion equivocada. es por eso que una manera de mejorar es generalizar la estrategia anterior y usar validacion cruzada de k pliegues.

la idea es la misma, usar un conjunto de entrenamiento para contruir el modelo y otro de prueba para evaluarlo, pero en lugar de hacerlo una vez, se hace k veces, donde cada vez se elige un subconjunto de observaciones para el conjunto de prueba y el resto para el conjunto de entrenamiento.

obtenemos las estimaciones de error de la sigguiente manera:

1. Para cada uno de los k subconjuntos:
  a) tomar uno de los k subconjutnos del conjunto de entreamiento y reservalo como conjunto de prueba.
  b) ajustar la recta de minimos cuadrados usando para ello los datos combinados de los k-1 subconjuntos restantes.
  c)Estimar el error cuadratico medio usando para ellos el conjunto de prueba

2. Estimar el error cuadratico medio, corresponeidnte a la media de los k MSE obtenidos en el paso 1.
